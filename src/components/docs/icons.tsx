...
export function IconsCatalog() {
  ...
  const [view, setView] = React.useState<'all' | 'legacy' | 'new'>('all')
  ...
  const derived = React.useMemo(() => {
    ...
    const newSet = new Set(NEW_ICON_KEYS as readonly string[])
    const legacy = all.filter(n => !newSet.has(n))
    const inView = view === 'new' ? [...newSet].sort((a,b)=>a.localeCompare(b)) : view === 'legacy' ? legacy : all
    ...
    const counts = {
      ...
      new: q ? [...newSet].filter(n => n.toLowerCase().includes(q)).length : newSet.size,
      legacy: q ? legacy.filter(n => n.toLowerCase().includes(q)).length : legacy.length,
    }
        ...
        <div className="flex flex-col gap-3 border-b border-border/60 bg-card p-3 rounded-t-xl sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="inline-flex overflow-hidden rounded-md border">
              {(['all','legacy','new'] as const).map(val => (
                <button
                  key={val}
                  type="button"
                  onClick={() => setView(val)}
                ...
                >
                  {val === 'all' ? 'All' : val === 'legacy' ? 'Legacy' : 'New'}

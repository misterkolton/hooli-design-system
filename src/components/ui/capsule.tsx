...
function mapLegacyBg(bg?: LegacyBackgroundColor) {
...
function mapLegacyTextColor(color?: LegacyTextColor): string | undefined {
  switch (color) {
    case 'inverseBody':
    case 'inverseHeading':
...
export const Capsule = React.forwardRef<HTMLDivElement, CapsuleProps>(
  (
    ...
    const derivedTone = tone ?? mapLegacyBg(backgroundColor)
    const legacyText = mapLegacyTextColor(fontColor)
    ...
    return (
      <div
        ref={ref}
        className={cn(capsuleVariants({ tone: derivedTone, size, shadow: shadowVariant }), legacyText, className)}
        {...props}
      >
        {title ?? children}
      </div>

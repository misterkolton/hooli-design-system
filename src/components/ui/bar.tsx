...
type LegacyBackgroundColor = { swatch?: string; shade?: string } | string
...
function mapLegacyBgToClass(bg?: LegacyBackgroundColor): string | undefined {
...
export const Bar = React.forwardRef<HTMLDivElement, BarProps>(
  (
    ...
    const legacyBgClass = mapLegacyBgToClass(backgroundColor)
    const styleBg = typeof backgroundColor === 'string' ? { backgroundColor } : {}

    const toneClass = legacyBgClass
      ? legacyBgClass
      : tone === 'primary'
        ? 'bg-primary'
        : tone === 'secondary'
